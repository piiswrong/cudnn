...
#undef GMTK_ARGUMENTS_CHECK_ARGS
  ...
  int *layerDims = new int[numLayers+1];
  layerDims[0] = inputSize;
  
  for (unsigned i=0; i < numLayers; i+=1)
    layerDims[i+1] = (int)dnn->layerOutputs(i);

  bool warned = false;
  DNeuron<GPU_REAL> **neurons = new DNeuron<GPU_REAL>*[numLayers];
  for (unsigned i=0; i < numLayers; i+=1) {
	...
	error("ERROR: gmtkDMLPtrain only supports softmax for the output layer\n");
      ...
      neurons[i] = new DSoftmaxNeuron<GPU_REAL>(bpMiniBatchSize , handle);
      break;
	...
	error("ERROR: gmtkDMLPtrain only supports linear or softmax for the output layer\n");
      ...
      error("ERROR: tanh is not yet supported\n");
      break;
    case DeepNN::ODDROOT: 
      if (i == numLayers - 1) {
	error("ERROR: gmtkDMLPtrain only supports linear or softmax for the output layer\n");
      }
      neurons[i] = new DOddrootNeuron<GPU_REAL>(handle);
      break;
    case DeepNN::LINEAR:
      neurons[i] = new DNeuron<GPU_REAL>(handle);
      break;
    case DeepNN::RECTLIN:
      if (i == numLayers - 1) {
	error("ERROR: gmtkDMLPtrain only supports linear or softmax for the output layer\n");
	...
	warned = true;
      ...
      neurons[i] = new DReLUNeuron<GPU_REAL>(handle);
      break;
    default: 
...
/*  vector<AllocatingMatrix> W(numLayers); // copy existing weights into W & B in case we're resuming training
  ...
  DHyperParams ptHyperParams, bpHyperParams;
  ptHyperParams.learning_rate           = ptInitStepSize;
  ptHyperParams.momentum                = ptMinMomentum;
  ptHyperParams.max_momentum            = ptMaxMomentum;
  ptHyperParams.decay_rate              = ptL2;
  if (ptL2 > 0) 
      ptHyperParams.weight_decay        = true;
  else
      ptHyperParams.weight_decay        = false;
  ptHyperParams.batch_size              = ptMiniBatchSize;
  ptHyperParams.check_interval          = ptCheckInterval;
  ptHyperParams.idrop_out               = false;
  ptHyperParams.idrop_rate              = 0.0;
  ptHyperParams.hdrop_out               = false;
  ptHyperParams.hdrop_rate              = 0.0;

  bpHyperParams.learning_rate           = bpInitStepSize;
  bpHyperParams.momentum                = bpMinMomentum;
  bpHyperParams.max_momentum            = bpMaxMomentum;
  bpHyperParams.decay_rate              = bpL2;
  if (ptL2 > 0)
      bpHyperParams.weight_decay        = true;
  else
      bpHyperParams.weight_decay        = false;
  bpHyperParams.batch_size              = bpMiniBatchSize;
  bpHyperParams.check_interval          = bpCheckInterval;
  bpHyperParams.idrop_rate              = bpIdropP;
  if (bpIdropP > 0.0)
      bpHyperParams.idrop_out           = true;
  else
      bpHyperParams.idrop_out           = false;
  bpHyperParams.hdrop_rate              = bpHdropP;
  if (bpHdropP > 0.0)
      bpHyperParams.hdrop_out           = true;
  else 
  ...
  for (unsigned layer=0; layer < numLayers; layer+=1) {
    DMatrix<GPU_REAL> *m = layers[layer]->weight();
    int ld = m->ncols()-1, fd = m->nrows() - 1;
    double *w = new double[ld*fd];
    double *b = new double[ld];
    for (int r = 0; r < ld; r++) {
        for (int c = 0; c < fd; c++) {
            w[c*ld + r] = m->getElem(c, r);
        }
        b[r] = m->getElem(fd, r);
    }
    dnn->setParams(layer, w, ld, b);
  }
