...
def test(name, id):
    ...
    for exp in id:
        ...
        head = '%s_%d_'%(name,exp)
        tail = '.param'
        params = [ fname for fname in files if fname.startswith(head) and fname.endswith(tail)]
        x = [ int(i[len(head):-len(tail)]) for i in params ]
        testAcc = [ 0 for i in xrange(len(params)) ]
...
matrices:""")
        ...
        for xx, param_name in zip(x, params):
